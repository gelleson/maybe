...
namespace :securities do
  desc "Backfill exchange_operating_mic for securities (DISABLED - no securities provider)"
  task backfill_exchange_mic: :environment do
    puts "ERROR: Securities backfill is disabled. No securities data provider is configured."
    puts "This task previously used Synth API which has been removed."
    exit 1
  end
